...
class stockTransport(models.Model):
    ...
    per_volume = fields.Float(compute="_compute_volume",string="Volume",store=True, help="total volume calculated for progressbar")
    per_weight = fields.Float(compute="_compute_weight",string="Weight",store=True, help="total weight calculated for progressbar")
    ...
    tranfer_count = fields.Float(compute="_tranfer_count", store=True)
    lines_count = fields.Float(compute="_lines_count", store=True)
    ...
    def _tranfer_count(self):
        for record in self:
            record.tranfer_count = len(record.picking_ids)
